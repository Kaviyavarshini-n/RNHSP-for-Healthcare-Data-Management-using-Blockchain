...
# PBFT simulation for data validation
def pbft_consensus(data):
    print("Initiating PBFT consensus...")
    time.sleep(1)  # Simulate time for consensus
    nodes = [f"PBFT consensus{i+1}" for i in range(4)]  
    approvals = {node: random.choice([True, True, False]) for node in nodes}  

    print("Node approvals:")
    for node, approval in approvals.items():
        print(f"{node}: {'Approved' if approval else 'Rejected'}")

    if list(approvals.values()).count(True) >= 2:  
        print("PBFT consensus reached: Data validated.")
        return True
    else:
        print("PBFT consensus failed: Data rejected.")
        return False
    ...
    if not ac.check_permission(user_role, "read"):
        ...
        if multi_factor_authentication(user_role, password, generated_password, user_passwords):
            ...
            if user_role == "admin":
                ...
                if pbft_consensus(doctor_records):
                    ipfs_hash = upload_to_ipfs(json.dumps(doctor_records))
                    print(f"Doctor data uploaded to IPFS at hash: {ipfs_hash}")
                ...
                for record in data:
                    if record['Name'] == user_id:
                        ...
                        if pbft_consensus(patient_data):
                            ipfs_hash = upload_to_ipfs(json.dumps(patient_data))
                            print(f"Patient data uploaded to IPFS at hash: {ipfs_hash}")
                        break
                else:
