 ...
 */
class IotIphcImplTest : public TestCase
{
...
IotIphcImplTest::IotIphcImplTest ()
  : TestCase ("Iot implementation")
{
}

void IotIphcImplTest::ReceivePacket (Ptr<Socket> socket, Ptr<Packet> packet, const Address &from)
{
  m_receivedPacket = packet;
}

void IotIphcImplTest::ReceivePkt (Ptr<Socket> socket)
{
...
void
IotIphcImplTest::DoSendData (Ptr<Socket> socket, std::string to)
{
...
void
IotIphcImplTest::SendData (Ptr<Socket> socket, std::string to)
{
  m_receivedPacket = Create<Packet> ();
  Simulator::ScheduleWithContext (socket->GetNode ()->GetId (), Seconds (0),
                                  &IotIphcImplTest::DoSendData, this, socket, to);
  Simulator::Run ();
}

void
IotIphcImplTest::DoRun (void)
{
  // Create topology
  ...
  { // first interface
    ...
    Ptr<IotNetDevice> rxSix = CreateObject<IotNetDevice> ();
  ...
  Ptr<SimpleNetDevice> txDev;
  {
    ...
    Ptr<IotNetDevice> txSix = CreateObject<IotNetDevice> ();
  ...
  rxSocket->SetRecvCallback (MakeCallback (&IotIphcImplTest::ReceivePkt, this));
 ...
 */
class IotIphcTestSuite : public TestSuite
{
public:
  IotIphcTestSuite ();
private:
...
IotIphcTestSuite::IotIphcTestSuite ()
  : TestSuite ("iot-iphc", UNIT)
{
  AddTestCase (new IotIphcImplTest (), TestCase::QUICK);
}

static IotIphcTestSuite g_iotIphcTestSuite; //!< Static variable for test initialization
