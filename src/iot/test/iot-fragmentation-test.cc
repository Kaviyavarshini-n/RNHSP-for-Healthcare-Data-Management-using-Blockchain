 ...
 */
class iotFragmentationTest : public TestCase
{
...
iotFragmentationTest::iotFragmentationTest ()
...
void
iotFragmentationTest::StartServer (Ptr<Node> serverNode)
{
  ...
  m_socketServer->SetRecvCallback (MakeCallback (&iotFragmentationTest::HandleReadServer, this));
}

void
iotFragmentationTest::HandleReadServer (Ptr<Socket> socket)
{
  Ptr<Packet> packet;
  Address from;
...
void
iotFragmentationTest::StartClient (Ptr<Node> clientNode)
{
  ...
  if (m_socketClient == 0)
    {
      ...
      CallbackValue cbValue = MakeCallback (&iotFragmentationTest::HandleReadIcmpClient, this);
      m_socketClient->SetAttribute ("IcmpCallback6", cbValue);
    }

  m_socketClient->SetRecvCallback (MakeCallback (&iotFragmentationTest::HandleReadClient, this));
}

void
iotFragmentationTest::HandleReadClient (Ptr<Socket> socket)
{
  Ptr<Packet> packet;
  Address from;
  while ((packet = socket->RecvFrom (from)))
...
void
iotFragmentationTest::HandleReadIcmpClient (Ipv6Address icmpSource,
...
void
iotFragmentationTest::SetFill (uint8_t *fill, uint32_t fillSize, uint32_t dataSize)
{
...
Ptr<Packet> iotFragmentationTest::SendClient (void)
{
  Ptr<Packet> p;
  if (m_dataSize)
    {
...
void
iotFragmentationTest::DoRun (void)
{
  // Create topology
  ...
  Ptr<BinaryErrorModel> serverDevErrorModel = CreateObject<BinaryErrorModel> ();
  {
    ...
    Ptr<IoTNetDevice> serverSix = CreateObject<IoTNetDevice> ();
  ...
  Ptr<BinaryErrorModel> clientDevErrorModel = CreateObject<BinaryErrorModel> ();
  {
    ...
    Ptr<IoTNetDevice> clientSix = CreateObject<IoTNetDevice> ();
  ...
  for ( int i = 0; i < 5; i++)
    {
      ...
      Simulator::ScheduleWithContext (m_socketClient->GetNode ()->GetId (), Seconds (0),
                                      &iotFragmentationTest::SendClient, this);
      Simulator::Run ();
 ...
 */
class iotFragmentationTestSuite : public TestSuite
{
public:
  iotFragmentationTestSuite ();
private:
};

iotFragmentationTestSuite::iotFragmentationTestSuite ()
  : TestSuite ("iot-fragmentation", UNIT)
{
  AddTestCase (new iotFragmentationTest (), TestCase::QUICK);
}

static iotFragmentationTestSuite g_iotFragmentationTestSuite; //!< Static variable for test initialization
