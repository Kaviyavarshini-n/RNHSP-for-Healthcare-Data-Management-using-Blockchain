...
The source code for the iot module lives in the directory ``src/iot``.
...
This is possible though the ``IoTHelper::AddContext`` function.
...
* Tx - exposing packet (including 6LoWPAN header), IoTNetDevice Ptr, interface index.
* Rx - exposing packet (including 6LoWPAN header), IoTNetDevice Ptr, interface index.
* Drop - exposing DropReason, packet (including 6LoWPAN header), IoTNetDevice Ptr, interface index.
...
Add ``iot`` to the list of modules built with |ns3|.
...
The following example can be found in ``src/iot/examples/``:

* ``example-iot.cc``:  A simple example showing end-to-end data transfer.
