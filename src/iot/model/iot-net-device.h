...
#ifndef IOT_NET_DEVICE_H
#define IOT_NET_DEVICE_H
 ...
 */
class IoTNetDevice : public NetDevice
{
public:
  /**
   * Enumeration of the dropping reasons in IoT.
   */
  enum DropReason
  {
  ...
  /**
   * Constructor for the IoTNetDevice.
   */
  IoTNetDevice ();
  ...
  /**
   * \brief Setup IoT to be a proxy for the specified NetDevice.
   * All the packets incoming and outgoing from the NetDevice will be
   * processed by IoTNetDevice.
   *
   ...
   * \param [in] sixNetDevice The IoTNetDevice.
   * \param [in] ifindex The ifindex of the device.
   * \deprecated The non-const \c Ptr<IoTNetDevice> argument
   * is deprecated and will be changed to \c Ptr<const IoTNetDevice>
   * in a future release.
   */
  typedef void (* RxTxTracedCallback)(Ptr<const Packet> packet,
                                      Ptr<IoTNetDevice> sixNetDevice,
                                      uint32_t ifindex);
...
private:
   ...
   */
  IoTNetDevice (IoTNetDevice const &);
  /**
   * \brief Copy constructor
   *
   * Defined and unimplemented to avoid misuse.
   * \returns
   */
  IoTNetDevice& operator= (IoTNetDevice const &);
  /**
   ...
   * \li Ptr to IoTNetDevice
   * \li interface index
   * \deprecated The non-const \c Ptr<IoTNetDevice> argument
   * is deprecated and will be changed to \c Ptr<const IoTNetDevice>
   * in a future release.
   */
  TracedCallback<Ptr<const Packet>, Ptr<IoTNetDevice>, uint32_t> m_txTrace;
   ...
   * in a future release.
   */
  TracedCallback<Ptr<const Packet>, Ptr<IoTNetDevice>, uint32_t> m_rxTrace;
   ...
   * in a future release.
   */
  TracedCallback<DropReason, Ptr<const Packet>, Ptr<IoTNetDevice>, uint32_t> m_dropTrace;
