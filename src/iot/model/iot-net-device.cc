...
TypeId IoTNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::IoTNetDevice")
    ...
    .AddAttribute ("MeshUnderJitter",
                   ...
                   MakePointerAccessor (&IoTNetDevice::m_meshUnderJitter),
                   MakePointerChecker<RandomVariableStream> ())
    .AddTraceSource ("Tx",
                     "Send - packet (including 6LoWPAN header), "
                     "IoTNetDevice Ptr, interface index.",
                     MakeTraceSourceAccessor (&IoTNetDevice::m_txTrace),
                     "ns3::IoTNetDevice::RxTxTracedCallback")
    .AddTraceSource ("Rx",
                     "Receive - packet (including 6LoWPAN header), "
                     "IoTNetDevice Ptr, interface index.",
                     MakeTraceSourceAccessor (&IoTNetDevice::m_rxTrace),
                     "ns3::IoTNetDevice::RxTxTracedCallback")
    .AddTraceSource ("Drop",
                     "Drop - DropReason, packet (including 6LoWPAN header), "
                     "IoTNetDevice Ptr, interface index.",
                     MakeTraceSourceAccessor (&IoTNetDevice::m_dropTrace),
                     "ns3::IoTNetDevice::DropTracedCallback")
  ;
  return tid;
}
...
Ptr<NetDevice> IoTNetDevice::GetNetDevice () const
{
  NS_LOG_FUNCTION (this);
  return m_netDevice;
}

void IoTNetDevice::SetNetDevice (Ptr<NetDevice> device)
{
  ...
  m_node->RegisterProtocolHandler (MakeCallback (&IoTNetDevice::ReceiveFromDevice,
                                                 this),
                                   protocolType, device, false);
}

int64_t IoTNetDevice::AssignStreams (int64_t stream)
{
...
void IoTNetDevice::ReceiveFromDevice (Ptr<NetDevice> incomingPort,
                                            ...
                                            PacketType packetType)
{
  ...
  IoTDispatch::Dispatch_e dispatchVal;
  Ptr<Packet> copyPkt = packet->Copy ();

  m_rxTrace (copyPkt, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());

  copyPkt->CopyData (&dispatchRawVal, sizeof(dispatchRawVal));
  dispatchVal = IoTDispatch::GetDispatchType (dispatchRawVal);
  ...
  if ( dispatchVal == IoTDispatch::LOWPAN_MESH )
    {
      hasMesh = true;
  ...
  if ( dispatchVal == IoTDispatch::LOWPAN_BC0 )
    {
      hasBc0 = true;
  ...
  if (hasMesh)
    {
      if (!hasBc0)
        {
          ...
          m_dropTrace (DROP_UNKNOWN_EXTENSION, copyPkt, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
          return;
        }
      ...
      NS_ABORT_MSG_IF (!Mac16Address::IsMatchingType (meshHdr.GetFinalDst ()), "IoT mesh-under flooding can not currently handle extended address final destinations: " << meshHdr.GetFinalDst ());
      NS_ABORT_MSG_IF (!Mac48Address::IsMatchingType (m_netDevice->GetAddress ()), "IoT mesh-under flooding can not currently handle devices using extended addresses: " << m_netDevice->GetAddress ());
  ...
  if ( dispatchVal == IoTDispatch::LOWPAN_FRAG1 )
    {
      isPktDecompressed = ProcessFragment (copyPkt, realSrc, realDst, true);
      fragmented = true;
    }
  else if ( dispatchVal == IoTDispatch::LOWPAN_FRAGN )
    {
  ...
  switch ( dispatchVal )
    {
    case IoTDispatch::LOWPAN_IPv6:
      NS_LOG_DEBUG ( "Packet without compression. Length: " << copyPkt->GetSize () );
      {
        IoTIpv6 uncompressedHdr;
        copyPkt->RemoveHeader (uncompressedHdr);
        isPktDecompressed = true;
      }
      break;
    case IoTDispatch::LOWPAN_HC1:
      if (m_useIphc)
        {
          m_dropTrace (DROP_DISALLOWED_COMPRESSION, copyPkt, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
          return;
        }
      DecompressLowPanHc1 (copyPkt, realSrc, realDst);
      isPktDecompressed = true;
      break;
    case IoTDispatch::LOWPAN_IPHC:
      if (!m_useIphc)
        {
          m_dropTrace (DROP_DISALLOWED_COMPRESSION, copyPkt, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
          return;
      ...
      if (DecompressLowPanIphc (copyPkt, realSrc, realDst))
        {
          m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, copyPkt, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
        }
      else
        {
          isPktDecompressed = true;
...
void IoTNetDevice::SetIfIndex (const uint32_t index)
{
  NS_LOG_FUNCTION (this << index);
  m_ifIndex = index;
}

uint32_t IoTNetDevice::GetIfIndex (void) const
{
  NS_LOG_FUNCTION (this);
  return m_ifIndex;
}

Ptr<Channel> IoTNetDevice::GetChannel (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );

  return m_netDevice->GetChannel ();
}

void IoTNetDevice::SetAddress (Address address)
{
...
Address IoTNetDevice::GetAddress (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
bool IoTNetDevice::SetMtu (const uint16_t mtu)
{
...
uint16_t IoTNetDevice::GetMtu (void) const
{
  NS_LOG_FUNCTION (this);
...
bool IoTNetDevice::IsLinkUp (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
void IoTNetDevice::AddLinkChangeCallback (Callback<void> callback)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
bool IoTNetDevice::IsBroadcast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
Address IoTNetDevice::GetBroadcast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
bool IoTNetDevice::IsMulticast (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
Address IoTNetDevice::GetMulticast (Ipv4Address multicastGroup) const
{
...
bool IoTNetDevice::IsPointToPoint (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
bool IoTNetDevice::IsBridge (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
bool IoTNetDevice::Send (Ptr<Packet> packet,
                               const Address& dest,
...
bool IoTNetDevice::SendFrom (Ptr<Packet> packet,
                                   const Address& src,
                                   const Address& dest,
                                   uint16_t protocolNumber)
{
...
bool IoTNetDevice::DoSend (Ptr<Packet> packet,
                                 const Address& src,
                                 const Address& dest,
                                 uint16_t protocolNumber,
                                 bool doSendFrom)
  ...
  if (pktSize > m_netDevice->GetMtu ())
    {
      ...
      for ( it = fragmentList.begin (); it != fragmentList.end (); it++ )
        {
          NS_LOG_DEBUG ( "IoTNetDevice::Send (Fragment) " << **it );
          m_txTrace (*it, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
    ...
    {
      m_txTrace (packet, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
      ...
      if (doSendFrom)
        {
          NS_LOG_DEBUG ( "IoTNetDevice::SendFrom " << m_node->GetId () << " " << *packet );
          ret = m_netDevice->SendFrom (packet, src, destination, protocolNumber);
        }
      else
        {
          NS_LOG_DEBUG ( "IoTNetDevice::Send " << m_node->GetId () << " " << *packet );
...
Ptr<Node> IoTNetDevice::GetNode (void) const
{
  NS_LOG_FUNCTION (this);
  return m_node;
}

void IoTNetDevice::SetNode (Ptr<Node> node)
{
  NS_LOG_FUNCTION (this << node);
  m_node = node;
}

bool IoTNetDevice::NeedsArp (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG ( m_netDevice != 0, "IoT: can't find any lower-layer protocol " << m_netDevice );
...
void IoTNetDevice::SetReceiveCallback (NetDevice::ReceiveCallback cb)
{
  NS_LOG_FUNCTION (this << &cb);
  m_rxCallback = cb;
}

void IoTNetDevice::SetPromiscReceiveCallback (NetDevice::PromiscReceiveCallback cb)
{
  NS_LOG_FUNCTION (this << &cb);
  m_promiscRxCallback = cb;
}

bool IoTNetDevice::SupportsSendFrom () const
{
  NS_LOG_FUNCTION (this);
  return true;
}
...
uint32_t
IoTNetDevice::CompressLowPanHc1 (Ptr<Packet> packet, Address const &src, Address const &dst)
{
  ...
  if ( packet->PeekHeader (ipHeader) != 0 )
    {
      ...
      if (srcAddr.IsLinkLocal () && isSrcSrc )
        {
          hc1Header.SetSrcCompression (IoTHc1::HC1_PCIC);
        }
      else if (srcAddr.IsLinkLocal () )
        {
          hc1Header.SetSrcCompression (IoTHc1::HC1_PCII);
          hc1Header.SetSrcInterface (bufOne + 8);
        }
      else if ( isSrcSrc )
        {
          hc1Header.SetSrcCompression (IoTHc1::HC1_PIIC);
          hc1Header.SetSrcPrefix (bufOne);
        }
      else
        {
          hc1Header.SetSrcCompression (IoTHc1::HC1_PIII);
          hc1Header.SetSrcInterface (bufOne + 8);
          hc1Header.SetSrcPrefix (bufOne);
        }
      ...
      if (dstAddr.IsLinkLocal () && isDstDst )
        {
          hc1Header.SetDstCompression (IoTHc1::HC1_PCIC);
        }
      else if (dstAddr.IsLinkLocal () )
        {
          hc1Header.SetDstCompression (IoTHc1::HC1_PCII);
          hc1Header.SetDstInterface (bufOne + 8);
        }
      else if ( isDstDst )
        {
          hc1Header.SetDstCompression (IoTHc1::HC1_PIIC);
          hc1Header.SetDstPrefix (bufOne);
        }
      else
        {
          hc1Header.SetDstCompression (IoTHc1::HC1_PIII);
          hc1Header.SetDstInterface (bufOne + 8);
          hc1Header.SetDstPrefix (bufOne);
        }
...
void
IoTNetDevice::DecompressLowPanHc1 (Ptr<Packet> packet, Address const &src, Address const &dst)
{
  NS_LOG_FUNCTION (this << *packet << src << dst);
...
uint32_t
IoTNetDevice::CompressLowPanIphc (Ptr<Packet> packet, Address const &src, Address const &dst)
{
  NS_LOG_FUNCTION (this << *packet << src << dst);
  ...
  if ( packet->PeekHeader (ipHeader) != 0 )
    {
      ...
      if ( (ipHeader.GetFlowLabel () == 0) && (ipHeader.GetTrafficClass () == 0) )
        {
          iphcHeader.SetTf (IoTIphc::TF_ELIDED);
        }
      else if ( (ipHeader.GetFlowLabel () != 0) && (ipHeader.GetTrafficClass () != 0) )
        {
          iphcHeader.SetTf (IoTIphc::TF_FULL);
        ...
        {
          iphcHeader.SetTf (IoTIphc::TF_FL_ELIDED);
          iphcHeader.SetEcn ( (ipHeader.GetTrafficClass () & 0xC0) >> 6);
          iphcHeader.SetDscp ( ipHeader.GetTrafficClass () & 0x3F );
        }
      else
        {
          iphcHeader.SetTf (IoTIphc::TF_DSCP_ELIDED);
          iphcHeader.SetEcn ( (ipHeader.GetTrafficClass () & 0xC0) >> 6);
          iphcHeader.SetFlowLabel (ipHeader.GetFlowLabel ());
        }
      ...
      if (ipHeader.GetHopLimit () == 1)
        {
          iphcHeader.SetHlim (IoTIphc::HLIM_COMPR_1);
        }
      else if (ipHeader.GetHopLimit () == 0x40)
        {
          iphcHeader.SetHlim (IoTIphc::HLIM_COMPR_64);
        }
      else if (ipHeader.GetHopLimit () == 0xFF)
        {
          iphcHeader.SetHlim (IoTIphc::HLIM_COMPR_255);
        }
      else
        {
          iphcHeader.SetHlim (IoTIphc::HLIM_INLINE);
          // Set the HopLimit
          ...
          if (srcAddr == Ipv6Address::GetAny ())
            {
              ...
              iphcHeader.SetSam (IoTIphc::HC_INLINE);
          ...
          // Check if the address can be compressed with stateful compression
          else if ( FindUnicastCompressionContext (srcAddr, srcContextId) )
            {
              ...
              if (Ipv6Address::MakeAutoconfiguredAddress (src, m_contextTable[srcContextId].contextPrefix) == srcAddr)
                {
                  iphcHeader.SetSam (IoTIphc::HC_COMPR_0);
                }
              else
                {
                  ...
                  if ( serializedCleanedAddress[8] == 0x00 && serializedCleanedAddress[9] == 0x00 &&
                    ...
                    {
                      iphcHeader.SetSam (IoTIphc::HC_COMPR_16);
                      iphcHeader.SetSrcInlinePart (serializedCleanedAddress+14, 2);
                    }
                  else
                    {
                      iphcHeader.SetSam (IoTIphc::HC_COMPR_64);
              ...
              if ( FindUnicastCompressionContext (dstAddr, dstContextId) )
                {
                  ...
                  if (Ipv6Address::MakeAutoconfiguredAddress (dst, m_contextTable[dstContextId].contextPrefix) == dstAddr)
                    {
                      iphcHeader.SetDam (IoTIphc::HC_COMPR_0);
                    }
                  else
                    {
                      ...
                      if ( serializedCleanedAddress[8] == 0x00 && serializedCleanedAddress[9] == 0x00 &&
                          ...
                          serializedCleanedAddress[12] == 0xfe && serializedCleanedAddress[13] == 0x00 )
                        {
                          iphcHeader.SetDam (IoTIphc::HC_COMPR_16);
                          iphcHeader.SetDstInlinePart (serializedCleanedAddress+14, 2);
                        }
                      else
                        {
                          iphcHeader.SetDam (IoTIphc::HC_COMPR_64);
              ...
              else
                {
                  ...
                  if ( dstAddr == Ipv6Address::MakeAutoconfiguredLinkLocalAddress (dst) )
                  ...
                  else
                    {
                      ...
                      iphcHeader.SetDam (IoTIphc::HC_INLINE);
                    }
                }
            }
          else
...
bool
IoTNetDevice::CanCompressLowPanNhc (uint8_t nextHeader)
{
  bool ret = false;
...
bool
IoTNetDevice::DecompressLowPanIphc (Ptr<Packet> packet, Address const &src, Address const &dst)
{
  NS_LOG_FUNCTION (this << *packet << src << dst);
  ...
  if ( encoding.GetSac () )
    {
      ...
      if ( encoding.GetSam () == IoTIphc::HC_INLINE )
        {
      ...
      else
        {
          ...
          if ( encoding.GetSam () == IoTIphc::HC_COMPR_64 )
            {
              memcpy (srcAddress +8, encoding.GetSrcInlinePart (), 8);
            }
          else if ( encoding.GetSam () == IoTIphc::HC_COMPR_16 )
            {
              srcAddress[11] = 0xff;
              srcAddress[12] = 0xfe;
              memcpy (srcAddress +14, encoding.GetSrcInlinePart (), 2);
            }
          else // IoTIphc::HC_COMPR_0
            {
  ...
  else
    {
      ...
      else if ( encoding.GetSam () == IoTIphc::HC_COMPR_64 )
        {
          uint8_t srcAddress[16] = { };
          memcpy (srcAddress +8, encoding.GetSrcInlinePart (), 8);
          srcAddress[0] = 0xfe;
  ...
  if ( encoding.GetDac () )
    {
      ...
      if ((encoding.GetDam () == IoTIphc::HC_INLINE  && !encoding.GetM ())
          || (encoding.GetDam () == IoTIphc::HC_COMPR_64  && encoding.GetM ())
          || (encoding.GetDam () == IoTIphc::HC_COMPR_16  && encoding.GetM ())
          || (encoding.GetDam () == IoTIphc::HC_COMPR_0  && encoding.GetM ()) )
        {
      ...
      if (encoding.GetM () == false)
        {
          ...
          if ( encoding.GetDam () == IoTIphc::HC_COMPR_64 )
            {
              memcpy (dstAddress +8, encoding.GetDstInlinePart (), 8);
            }
          else if ( encoding.GetDam () == IoTIphc::HC_COMPR_16 )
            {
              dstAddress[11] = 0xff;
              dstAddress[12] = 0xfe;
  ...
  else
    {
      ...
      if (encoding.GetM () == false)
        {
          // unicast
          if ( encoding.GetDam () == IoTIphc::HC_INLINE )
             {
               uint8_t dstAddress[16] = { };
               memcpy (dstAddress, encoding.GetDstInlinePart (), 16);
               ipHeader.SetDestination ( Ipv6Address::Deserialize (dstAddress) );
             }
          else if ( encoding.GetDam () == IoTIphc::HC_COMPR_64 )
            {
              uint8_t dstAddress[16] = { };
              memcpy (dstAddress +8, encoding.GetDstInlinePart (), 8);
              dstAddress[0] = 0xfe;
  ...
  switch (encoding.GetTf ())
    ...
    case IoTIphc::TF_FULL:
      ...
      break;
    case IoTIphc::TF_DSCP_ELIDED:
      traf |= encoding.GetEcn ();
      traf <<= 2;   // Add 2-bit pad
      ipHeader.SetTrafficClass (traf);
      ipHeader.SetFlowLabel (encoding.GetFlowLabel ());
      break;
    case IoTIphc::TF_FL_ELIDED:
      traf |= encoding.GetEcn ();
      traf = ( traf << 6 ) | encoding.GetDscp ();
      ipHeader.SetTrafficClass (traf);
      ipHeader.SetFlowLabel (0);
      break;
    case IoTIphc::TF_ELIDED:
      ipHeader.SetFlowLabel (0);
      ipHeader.SetTrafficClass (0);
      break;
    }
  ...
  if ( encoding.GetNh () )
    {
      // Next Header
      uint8_t dispatchRawVal = 0;
      IoTDispatch::NhcDispatch_e dispatchVal;

      packet->CopyData (&dispatchRawVal, sizeof(dispatchRawVal));
      dispatchVal = IoTDispatch::GetNhcDispatchType (dispatchRawVal);

      if (dispatchVal == IoTDispatch::LOWPAN_UDPNHC)
        {
...
uint32_t
IoTNetDevice::CompressLowPanNhc (Ptr<Packet> packet, uint8_t headerType, Address const &src, Address const &dst)
{
  NS_LOG_FUNCTION (this << *packet << int(headerType));

  IoTNhcExtension nhcHeader;
  uint32_t size = 0;
  Buffer blob;
  ...
  if (headerType == Ipv6Header::IPV6_EXT_HOP_BY_HOP)
    {
      ...
      nhcHeader.SetEid (IoTNhcExtension::EID_HOPBYHOP_OPTIONS_H);
  ...
  else if (headerType == Ipv6Header::IPV6_EXT_ROUTING)
    {
      ...
      nhcHeader.SetEid (IoTNhcExtension::EID_ROUTING_H);
  ...
  else if (headerType == Ipv6Header::IPV6_EXT_FRAGMENTATION)
    {
      ...
      nhcHeader.SetEid (IoTNhcExtension::EID_FRAGMENTATION_H);
  ...
  else if (headerType == Ipv6Header::IPV6_EXT_DESTINATION)
    {
      ...
      nhcHeader.SetEid (IoTNhcExtension::EID_DESTINATION_OPTIONS_H);
...
IoTNetDevice::DecompressLowPanNhc (Ptr<Packet> packet, Address const &src, Address const &dst, Ipv6Address srcAddress, Ipv6Address dstAddress)
{
  NS_LOG_FUNCTION (this << *packet);

  IoTNhcExtension encoding;
  ...
  switch (actualEncodedHeaderType)
    {
    case IoTNhcExtension::EID_HOPBYHOP_OPTIONS_H:
    ...
    case IoTNhcExtension::EID_ROUTING_H:
    ...
    case IoTNhcExtension::EID_FRAGMENTATION_H:
    ...
    case IoTNhcExtension::EID_DESTINATION_OPTIONS_H:
      ...
      break;
    case IoTNhcExtension::EID_MOBILITY_H:
      // \todo: IPv6 Mobility Header is not supported in ns-3
      NS_ABORT_MSG ("IPv6 Mobility Header is not supported in ns-3 yet");
      break;
    case IoTNhcExtension::EID_IPv6_H:
      actualHeaderType = Ipv6Header::IPV6_IPV6;
      if (DecompressLowPanIphc (packet, src, dst))
        {
          m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, packet, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
...
uint32_t
IoTNetDevice::CompressLowPanUdpNhc (Ptr<Packet> packet, bool omitChecksum)
{
  NS_LOG_FUNCTION (this << *packet << int(omitChecksum));

  UdpHeader udpHeader;
  IoTUdpNhcExtension udpNhcHeader;
  uint32_t size = 0;
  ...
  if ( (udpHeader.GetSourcePort () >> 4 ) == 0xf0b && (udpHeader.GetDestinationPort () >> 4 ) == 0xf0b )
    {
      udpNhcHeader.SetPorts (IoTUdpNhcExtension::PORTS_LAST_SRC_LAST_DST);
    }
  else if ( (udpHeader.GetSourcePort () >> 8 ) == 0xf0 && (udpHeader.GetDestinationPort () >> 8 ) != 0xf0 )
    {
      udpNhcHeader.SetPorts (IoTUdpNhcExtension::PORTS_LAST_SRC_ALL_DST);
    }
  else if ( (udpHeader.GetSourcePort () >> 8 ) != 0xf0 && (udpHeader.GetDestinationPort () >> 8 ) == 0xf0 )
    {
      udpNhcHeader.SetPorts (IoTUdpNhcExtension::PORTS_ALL_SRC_LAST_DST);
    }
  else
    {
      udpNhcHeader.SetPorts (IoTUdpNhcExtension::PORTS_INLINE);
    }
...
void
IoTNetDevice::DecompressLowPanUdpNhc (Ptr<Packet> packet, Ipv6Address saddr, Ipv6Address daddr)
{
  NS_LOG_FUNCTION (this << *packet);

  UdpHeader udpHeader;
  IoTUdpNhcExtension encoding;
...
void IoTNetDevice::DoFragmentation (Ptr<Packet> packet,
...
bool IoTNetDevice::ProcessFragment (Ptr<Packet>& packet, Address const &src, Address const &dst, bool isFirst)
{
  NS_LOG_FUNCTION ( this << *packet );
  IoTFrag1 frag1Header;
  IoTFragN fragNHeader;
  FragmentKey_t key;
  uint16_t packetSize;
  ...
  if ( isFirst )
    {
      ...
      IoTDispatch::Dispatch_e dispatchValFrag1;
      ...
      dispatchValFrag1 = IoTDispatch::GetDispatchType (dispatchRawValFrag1);
      ...
      switch ( dispatchValFrag1 )
        ...
        case IoTDispatch::LOWPAN_IPHC:
          if (DecompressLowPanIphc (p, src, dst))
            {
              m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, p, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
              return false;
            }
          break;
        default:
      ...
      if ( m_fragmentReassemblyListSize && (m_fragments.size () >= m_fragmentReassemblyListSize) )
        {
          ...
          for (std::list< Ptr<Packet> >::iterator fragIter = storedFragments.begin ();
            ...
            {
              m_dropTrace (DROP_FRAGMENT_BUFFER_FULL, *fragIter, m_node->GetObject<IoTNetDevice> (), GetIfIndex ());
            }
...
IoTNetDevice::Fragments::Fragments ()
{
  NS_LOG_FUNCTION (this);
  m_packetSize = 0;
}
...
void IoTNetDevice::Fragments::AddFragment (Ptr<Packet> fragment, uint16_t fragmentOffset)
{
...
void IoTNetDevice::Fragments::AddFirstFragment (Ptr<Packet> fragment)
{
...
bool IoTNetDevice::Fragments::IsEntire () const
{
  NS_LOG_FUNCTION (this);
...
Ptr<Packet> IoTNetDevice::Fragments::GetPacket () const
{
  NS_LOG_FUNCTION (this);
...
void IoTNetDevice::Fragments::SetPacketSize (uint32_t packetSize)
{
  NS_LOG_FUNCTION (this << packetSize);
  m_packetSize = packetSize;
}

std::list< Ptr<Packet> > IoTNetDevice::Fragments::GetFraments () const
{
...
void
IoTNetDevice::Fragments::SetTimeoutIter (FragmentsTimeoutsListI_t iter)
{
  m_timeoutIter = iter;
  return;
}

IoTNetDevice::FragmentsTimeoutsListI_t
IoTNetDevice::Fragments::GetTimeoutIter ()
{
  return m_timeoutIter;
}

void IoTNetDevice::HandleFragmentsTimeout (FragmentKey_t key, uint32_t iif)
{
  NS_LOG_FUNCTION (this);
  ...
  for (std::list< Ptr<Packet> >::iterator fragIter = storedFragments.begin ();
       fragIter != storedFragments.end (); fragIter++)
    {
      m_dropTrace (DROP_FRAGMENT_TIMEOUT, *fragIter, m_node->GetObject<IoTNetDevice> (), iif);
    }
  // clear the buffers
  it->second = 0;
...
Address IoTNetDevice::Get16MacFrom48Mac (Address addr)
{
...
IoTNetDevice::FragmentsTimeoutsListI_t IoTNetDevice::SetTimeout (FragmentKey_t key, uint32_t iif)
{
  if (m_timeoutEventList.empty ())
    {
      m_timeoutEvent = Simulator::Schedule (m_fragmentExpirationTimeout, &IoTNetDevice::HandleTimeout, this);
    }
  m_timeoutEventList.emplace_back (Simulator::Now () + m_fragmentExpirationTimeout, key, iif);

  IoTNetDevice::FragmentsTimeoutsListI_t iter = --m_timeoutEventList.end();

  return (iter);
}

void IoTNetDevice::HandleTimeout (void)
{
  ...
  m_timeoutEvent = Simulator::Schedule (difference, &IoTNetDevice::HandleTimeout, this);

  return;
}

void IoTNetDevice::AddContext (uint8_t contextId, Ipv6Prefix contextPrefix, bool compressionAllowed, Time validLifetime)
{
...
bool IoTNetDevice::GetContext (uint8_t contextId, Ipv6Prefix& contextPrefix, bool& compressionAllowed, Time& validLifetime)
{
...
void IoTNetDevice::RenewContext (uint8_t contextId, Time validLifetime)
{
...
void IoTNetDevice::InvalidateContext (uint8_t contextId)
{
  NS_LOG_FUNCTION (this << +contextId);
...
void IoTNetDevice::RemoveContext (uint8_t contextId)
{
  NS_LOG_FUNCTION (this << +contextId);
...
bool IoTNetDevice::FindUnicastCompressionContext (Ipv6Address address, uint8_t& contextId)
{
  NS_LOG_FUNCTION (this << address);
...
bool IoTNetDevice::FindMulticastCompressionContext (Ipv6Address address, uint8_t& contextId)
{
  NS_LOG_FUNCTION (this << address);
...
Ipv6Address IoTNetDevice::CleanPrefix (Ipv6Address address, Ipv6Prefix prefix)
{
  uint8_t addressBytes[16];
  address.GetBytes (addressBytes);
