 ...
 * \ingroup iot
 *
 * \brief Setup a iot stack to be used as a shim between IPv6 and a generic NetDevice.
 */
class IoTHelper
{
public:
  /*
   * Construct a IotHelper
   */
  IoTHelper ();
  /**
   * Set an attribute on each ns3::IotNetDevice created by
   * IotHelper::Install.
   *
  ...
  /**
   * \brief Install the Iot stack on top of an existing NetDevice.
   *
   ...
   * Iot devices will behave as shims between the NetDevices
   * passed in and IPv6.
   *
   ...
   *
   * \note IPv6 stack must be installed \a after Iot,
   * using the Iot NetDevices. See the example in the
   * examples directory.
   *
   *
   * \param [in] c The NetDevice container.
   * \return A container with the newly created IoTNetDevices.
   */
   ...
   *          IoTNetDevice should be modified to use a fixed stream.
